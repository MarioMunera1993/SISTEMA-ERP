...
const PrinterMaintenances = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                await maintenanceService.updatePrinterMaintenance(editingId, maintenanceData, selectedFile);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("¿Está seguro de eliminar este registro de mantenimiento?")) {
            try {
                await maintenanceService.deletePrinterMaintenance(id);
                loadAllMaintenances();
